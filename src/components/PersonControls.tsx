        ...
        <div className="space-y-6">
          {persons.map((person) => (
            <div key={person.id} className="space-y-3 p-4 bg-gray-50 rounded-lg">
              ...
              <Slider
                id={`slider-${person.id}`}
                value={[person.level]} // Properly control the slider value
                max={10}
                step={1}
                onValueChange={(value) => handleSliderChange(person.id, value)} // Make sure value updates here
                onValueCommit={(value) => handleSliderChange(person.id, value)} // Make sure value updates when commit happens
